...
import org.joshvm.j2me.dio.gpio.*;
import org.joshvm.j2me.dio.DeviceManager;
import org.joshvm.util.ByteBuffer;
import org.joshvm.j2me.directUI.*;
import org.joshvm.JEDI.displaydevice.SPILCDController.*;
...
public class ILI9341_driver extends SPILCDController {
	...
	public static final int DEFAULT_PIN_DC = 8;
	public static final int	DEFAULT_PIN_RST = 9;
	public static final int	DEFAULT_PIN_BLK = 6;
	public static final int DEFAULT_SPI_CONTROLLER = 0;
	public static final int DEFAULT_SPI_CS_ADDRESS = 0;
	public static final int DEFAULT_SPI_CLOCK_FREQUENCY = 20*1024*1024;
	...
	private void LCD_WR_DATA8(int da) throws Exception  //发送数据-8位参数
	{	//OLED_CS_Clr();
		OLED_DC_Set();
		LCD_Writ_Bus(da);  
		//OLED_CS_Set();
	}  
	private  void LCD_WR_DATA(int da) throws Exception 
	{// OLED_CS_Clr();
		OLED_DC_Set();
		LCD_Writ_Bus(da>>8);
		LCD_Writ_Bus(da);
	...
	private  void LCD_WR_DATA(ByteBuffer buff) throws Exception 
	{// OLED_CS_Clr();
		OLED_DC_Set();
		LCD_Writ_Bus(buff);
		//OLED_CS_Set();
	}
	private void LCD_WR_REG(int da)	  throws Exception 
	{	//	OLED_CS_Clr();
		OLED_DC_Clr();
		LCD_Writ_Bus(da);
		//OLED_CS_Set();
	}
	private  void LCD_WR_REG_DATA(int reg,int da) throws Exception 
	{	//OLED_CS_Clr();
		LCD_WR_REG(reg);
		LCD_WR_DATA(da);
		//OLED_CS_Set();
	...
	public void init(DisplayDeviceDescriptor desc) throws InvalidDeviceDescriptorException, IOException {
		try {
			if (!(desc instanceof ILI9341_DeviceDescriptor)) {
				throw new InvalidDeviceDescriptorException("The input DisplayDeviceDescriptor is not ILI9341_DeviceDescriptor");
			}
			
			super.init(desc);
				
			GPIOPinConfig cfg = new GPIOPinConfig(GPIOPinConfig.UNASSIGNED, 
													((ILI9341_DeviceDescriptor)desc).dc, //DC-PCM_CLK
			...
			cfg = new GPIOPinConfig(GPIOPinConfig.UNASSIGNED, 
													((ILI9341_DeviceDescriptor)desc).rst, //RST-PCM_SYNC
													GPIOPinConfig.DIR_OUTPUT_ONLY,
													GPIOPinConfig.MODE_OUTPUT_OPEN_DRAIN,
													GPIOPinConfig.TRIGGER_NONE,
													false);
			...
			cfg = new GPIOPinConfig(GPIOPinConfig.UNASSIGNED, 
															((ILI9341_DeviceDescriptor)desc).blk, //BLK-PCM_DIN
															GPIOPinConfig.DIR_OUTPUT_ONLY,
															GPIOPinConfig.MODE_OUTPUT_OPEN_DRAIN,
															GPIOPinConfig.TRIGGER_NONE,
															false);
	...
	public static SPILCDDeviceDescriptor getDeviceDescriptor (int SPIContollerNumber, int CSAddress, int clockFrequency) {
		return new ILI9341_DeviceDescriptor("com.joshvm.JEDI.LCDController.ILI9341_driver",
											SPIContollerNumber,
											CSAddress,
											clockFrequency,
											0, /*Clock mode 0*/
											8, /*Word length 8*/
											SPILCDDeviceDescriptor.BIG_ENDIAN,
											SPILCDDeviceDescriptor.CS_ACTIVE_LOW,
											DEFAULT_PIN_DC,
											DEFAULT_PIN_RST,
											DEFAULT_PIN_BLK);
	}
	...
	static class ILI9341_DeviceDescriptor extends SPILCDDeviceDescriptor {
		...
		ILI9341_DeviceDescriptor(String classNameOfFlashROMDriver,
												...
												int pin_rst,
												int pin_blk) {
				super(classNameOfFlashROMDriver,
						SPIControllerNumber, 
						CSAddress, 
						clockFrequency, 
						clockMode, 
